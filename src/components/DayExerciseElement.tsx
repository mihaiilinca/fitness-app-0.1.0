...
function DayExerciseElement(props) {
  ...
  useEffect(() => {
    const exerciseToFind = props.exercises.find(
      (obj) => obj._id === props.element.exerciseId
    );
    setExercise(exerciseToFind && exerciseToFind.name);
  }, [exercise]);
    ...
    exercise && (
      <div className="flex flex-row justify-between m-2">
        <div className="flex flex-row gap-x-1">
          <h1>{exercise}</h1>
          <h1>{`- ${props.element.timeInMinutes} Minuten`}</h1>
        </div>
        ...
        <div className="flex flex-row gap-x-3">
          <button
            ...
            onClick={() => {
              props.setShowPopupEditLabel("Übung");
              props.setShowPopupEdit(true);
              props.setExerciseElement(props.element);
            }}
          >
            <svg
...
export default DayExerciseElement;
